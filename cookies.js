...
if (cookieBox) {
  // Verifica si cookieBox existe
  const acceptBtn = cookieBox.querySelector("button");

  if (acceptBtn) {
    // Verifica si acceptBtn existe
    acceptBtn.onclick = () => {
      // Estableciendo la cookie por 1 mes, después de un mes expirará automáticamente
      document.cookie = "CookieBy=LumaraSEO; max-age=" + 60 * 60 * 24 * 30;
      if (document.cookie) {
        // Si la cookie está establecida
        cookieBox.classList.add("hide"); // Ocultar el cuadro de cookies
      } else {
        // Si la cookie no está establecida, entonces alertar un error
        alert(
  ...
  let checkCookie = document.cookie.indexOf("CookieBy=LumaraSEO"); // Revisando nuestra cookie
